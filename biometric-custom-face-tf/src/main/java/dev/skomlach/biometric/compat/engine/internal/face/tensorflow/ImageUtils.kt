...
object ImageUtils {
     ...
     */
    @JvmOverloads
    fun saveBitmap(context: Context, bitmap: Bitmap, filename: String = "preview.png") {
        val myDir =
            File(context.cacheDir, "tensorflow")
        if (!myDir.exists()) {
            if (!myDir.mkdirs()) {
                LogCat.log("Make dir failed")
                return
            }
        }
    ...
    @JvmOverloads
    fun deleteBitmap(context: Context, filename: String = "preview.png") {
        val myDir =
            File(context.cacheDir, "tensorflow")
        if (!myDir.exists()) {
            if (!myDir.mkdirs()) {
    ...
    fun getTransformationMatrix(
        ...
        if (applyRotation != 0) {
            if (applyRotation % 90 != 0) {
                LogCat.log("Rotation of %d % 90 != 0", applyRotation)
            }
