...
class SherpaOnnxBiometricManager(
    ...
    private fun authenticateWithStorage(
        ...
        if (trustIssue != null) {
            e("SherpaOnnxBiometricManager.authenticate rejected_untrusted_input=$trustIssue")
            finishWithError(
                callback,
                CUSTOM_BIOMETRIC_ERROR_UNABLE_TO_PROCESS,
                localized(R.string.biometriccompat_voice_help_sample_required)
        ...
        if (sampleQualityIssue != VoiceQualityIssue.NONE) {
            e("SherpaOnnxBiometricManager.authenticate quality=$sampleQualityIssue samples=${samples.size}")
            finishWithError(
                callback,
                CUSTOM_BIOMETRIC_ERROR_UNABLE_TO_PROCESS,
        ...
        if (hasMissingFingerprint || hasDuplicateSamples) {
            e(
                "SherpaOnnxBiometricManager.authenticate replay_risk=" +
        ...
        if (!isEnrollment && evaluateVoiceReplay(
            ...
            e("SherpaOnnxBiometricManager.authenticate replay_risk=recent_duplicate")
            finishWithError(
                callback,
                CUSTOM_BIOMETRIC_ERROR_UNABLE_TO_PROCESS,
                qualityMessage(VoiceQualityIssue.SAMPLE_REPLAY_RISK)
        ...
        if (matchingTemplates.isEmpty()) {
            ...
            e("SherpaOnnxBiometricManager.authenticate phrase_challenge=$phraseChallenge")
            finishWithError(
                callback,
                CUSTOM_BIOMETRIC_ERROR_UNABLE_TO_PROCESS,
        ...
        e(
            "SherpaOnnxBiometricManager.authenticate templates=${templates.size} " +
    ...
    internal companion object {
        ...
        private const val LOCKOUT_STORAGE_NAME = "sherpa_onnx_voice_lockout"
