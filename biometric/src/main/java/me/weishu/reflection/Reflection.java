package me.weishu.reflection;

import android.content.Context;
import android.os.StrictMode;
import android.text.TextUtils;
import android.util.Base64;

import java.io.File;
import java.io.FileOutputStream;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.List;

import dalvik.system.DexFile;

import static android.os.Build.VERSION.SDK_INT;

/**
 * @author weishu
 * @date 2018/6/7.
 */

public class Reflection {

    private static final String DEX = "";

    public static int unsealAll(Context context) {

        if (SDK_INT < 28) {
            // Below Android P, ignore
            return 0;
        }
        StrictMode.setVmPolicy(new StrictMode.VmPolicy.Builder(StrictMode.getVmPolicy()).permitNonSdkApiUsage().build());
        // try exempt API first.
        if (BootstrapClass.exemptAll()) {
            return 0;
        }
        if (unsealByDexFile(context, null)) {
            return 0;
        }

        return -1;
    }

    public static int unseal(Context context, List<String> packages) {
        if (SDK_INT < 28) {
            // Below Android P, ignore
            return 0;
        }
        List<String> strings = new ArrayList<>();
        for (String pkg : packages) {
            if (!TextUtils.isEmpty(pkg)) {
                if(pkg.endsWith("/"))
                    pkg = pkg.substring(0, pkg.length() - 1);
                strings.add("L" + pkg.replace(".", "/"));
            }
        }

        String[] methods = strings.toArray(new String[0]);
        StrictMode.setVmPolicy(new StrictMode.VmPolicy.Builder(StrictMode.getVmPolicy()).permitNonSdkApiUsage().build());
        // try exempt API
        if (BootstrapClass.exempt(methods)) {
            return 0;
        }
        if (unsealByDexFile(context, methods)) {
            return 0;
        }

        return -1;
    }

    private static boolean unsealByDexFile(Context context, String[] methods) {
        File codeCacheDir = context.getCacheDir();
        if (!codeCacheDir.exists())
            codeCacheDir.mkdirs();
        File code = new File(codeCacheDir, System.currentTimeMillis() + ".dex");
        try {
            try (FileOutputStream fos = new FileOutputStream(code)) {
                fos.write(Base64.decode(DEX, Base64.NO_WRAP));
            }
            DexFile dexFile = new DexFile(code);
            Class<?> bootstrapClass = dexFile.loadClass(BootstrapClass.class.getCanonicalName(), null);
            if (methods == null) {
                Method exemptAll = bootstrapClass.getDeclaredMethod("exemptAll");
                return (boolean) exemptAll.invoke(null);
            } else {
                Method exemptAll = bootstrapClass.getDeclaredMethod("exempt", String[].class);
                return (boolean) exemptAll.invoke(null, new Object[]{methods});
            }
        } catch (Throwable e) {
            e.printStackTrace();
            return false;
        } finally {
            if (code.exists()) {
                //noinspection ResultOfMethodCallIgnored
                code.delete();
            }
        }
    }
}