...
public class SoterFingerprintUnlockModule extends AbstractBiometricModule {

    private final BiometricInitListener listener;
    private BiometricManagerCompat manager;

    @SuppressLint("WrongConstant")
    public SoterFingerprintUnlockModule(final BiometricInitListener listener) {
        super(BiometricMethod.FINGERPRINT_SOTERAPI);

        this.listener = listener;
        try {
            manager = BiometricManagerCompat.from(getContext(), ConstantsSoter.FINGERPRINT_AUTH);
        } catch (Throwable ignore) {
            manager = null;
        }
        if (listener != null) {
            listener
                    .initFinished(BiometricMethod.FINGERPRINT_SOTERAPI, SoterFingerprintUnlockModule.this);
        }
    }
    ...
    public void authenticate(final CancellationSignal cancellationSignal,
        ...
        for (BiometricMethod method : BiometricMethod.values()) {
            if (method.getId() == tag()) {
                BiometricLoggerImpl.d("SoterFingerprintUnlockModule.authenticate - " + method.toString());
            }
        }
        ...
        if (manager != null) {
            try {
                ...
                BiometricLoggerImpl.e(e, "SoterFingerprintUnlockModule: authenticate failed unexpectedly");
            }
        }
    ...
    class AuthCallback extends BiometricManagerCompat.AuthenticationCallback {
        ...
        public void onAuthenticationError(int errMsgId, CharSequence errString) {
            BiometricLoggerImpl.d("SoterFingerprintUnlockModule.onAuthenticationError: " + CodeToString.getErrorCode(errMsgId) + "-" + errString);
            ...
            switch (errMsgId) {
                ...
                case BIOMETRIC_ERROR_USER_CANCELED:
                    Core.cancelAuthentication(SoterFingerprintUnlockModule.this);
                    return;
        ...
        public void onAuthenticationHelp(int helpMsgId, CharSequence helpString) {
            BiometricLoggerImpl.d("SoterFingerprintUnlockModule.onAuthenticationHelp: " + CodeToString.getHelpCode(helpMsgId) + "-" + helpString);
            if (listener != null) {
        ...
        public void onAuthenticationSucceeded(BiometricManagerCompat.AuthenticationResult result) {
            BiometricLoggerImpl.d("SoterFingerprintUnlockModule.onAuthenticationSucceeded: " + result);
            if (listener != null) {
        ...
        public void onAuthenticationFailed() {
            BiometricLoggerImpl.d("SoterFingerprintUnlockModule.onAuthenticationFailed: ");
            if (listener != null) {
