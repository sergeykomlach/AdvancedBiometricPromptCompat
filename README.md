...
- Dark/Light themes supported; Also you are able to get the background color of the current Biometric dialog
...
So, some tricks have been used that try to determine by indirect signs which biometric data are used (like "if NOT fingerprint, BUT something enrolled in the System Settings").
There are edge cases where we cannot tell exactly what type of biometrics is enrolled - for example, if it is Samsung with Face and Iris - in this case, the code can give an incorrect result. It can happen if you set ```BiometricApi.BIOMETRIC_API + BiometricType.BIOMETRIC_FACE``` or ```BiometricApi.BIOMETRIC_API + BiometricType.BIOMETRIC_IRIS```
